...
| Price history | For about **30 references, two or three dated `item_prices` rows inside the 180-day window** at `source = manual`, so a price genuinely moves mid-window. **Written before S4 generates sales**, because S4 stamps each line's `unit_price` from the row effective on that sale's own day — which is what produces real price/volume co-movement for S7 to fit rather than a stamped number. Without this the seeded tenant reads `insufficient_variation` on every reference, S7's elasticity engine is silent on the demo, and only the margin rule is ever seen. The movements are ordinary repricings, not a randomised experiment — S7 rules that out of v1 |
