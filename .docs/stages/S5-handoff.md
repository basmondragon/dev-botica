...
*Acceptance* says what must be true; this section is how a session ends. **Every check below runs on a tenant built by `seed_demo_tenant --profile default` and nothing else** (§1) — no client data, no client system, no row typed in by hand — because that is the state every session starts in. **The profile is named on every run because S0's command has no default**: a bare `seed_demo_tenant` refuses and prints the five names, so a session that leaves the flag off fails at its first command rather than three checks in, on a tenant whose shape nobody described. Checks 1 to 17 and check 19 run on `default`; **check 18 seeds its second tenant with `--profile minimal`**, the profile that exists so an isolation check has something to be isolated *from*. No check here reaches a state by editing rows — an unusual state is asked for by name or it is not reached at all, because a check that begins *first, delete some documents* is a check nobody runs twice (§1). Two targets carry every check and both ship with the stage: **the file export target**, and a **loopback HTTP target** that records every request it receives, holds what it accepts, and can be told to refuse, to hang, or to commit and then drop the connection. Neither leaves the instance. A check that needs a client's invoicing system is a check no build session can run, and there are none here (§11.1).

### Gates

| Gate | A failure means |
|---|---|
| Lint and format, server and client | style drift only — fix and move on; it blocks nothing else being read |
| Type-check, server and client | the generated API client and the endpoints disagree. Most often the `summary` body: its two shapes are a contract S9 is built against |
| The test suite | read the failing name before anything else — a failure under the delivery or builder names is a correctness failure in the one place this stage cannot be wrong; a failure elsewhere is usually a fixture that assumed a target was configured |
| Production build of the client | a work-list or settings route does not compile; the bundle checks below cannot be trusted until it does |
| Migration forward, then backward, on a `default` seed | if the reverse leaves `fiscal_document_status`, `fiscal_documents` or the partial index this stage migrated onto S4's `sales` (ledger rule 4) behind, S5 has permanently altered an earlier stage's table and cannot be rolled back on its own |
| `/api/openapi.json` regenerates and the client regenerates with no diff beyond S5's new paths | an endpoint's shape moved without the contract moving with it — the dangerous one is `summary` acquiring a count key in its unconfigured body |
| `seed_demo_tenant --profile default` builds from an empty database and again on top of itself with nothing changed, and `--profile minimal` completes beside it | S5's fixture is not idempotent, it assumes rows only a later stage contributes, or it answers one profile rather than the five §1 fixes. Every session after this one starts from this command, and check 18 has no second tenant to be isolated from |
| Every profile's tenant loads with an empty `invoicing` group and no `fiscal_documents` row | the default is not off, and everything in *Off when unconfigured* rests on it being off. A fixture that seeded a configured tenant would also make check 1 unrunnable, because the shipped silence is exactly what that check walks |
| Bundle grep for `Pendiente de envío`, `Enviado`, `Confirmado`, `Falló el envío` and `Envíos a facturación` | each must appear once as a literal. More than once means a second surface renders a status this document owns, and the two will diverge; zero means a label was translated or paraphrased at the point of use |

### Checks

1. **Silence, checked first because it is the default (A1, A2).** On the seed, close fifty sales across two sedes at the till. Then: `select count(*) from fiscal_documents` is 0; the queue table holds no job named `deliver_fiscal_document`, `sweep_fiscal_documents`, `export_fiscal_documents` or `notify_failed_deliveries`; `GET /api/fiscal-documents/summary` returns a body whose **only** key is `configured: false` — a body carrying `pending: 0` passes a count check and fails this one, and that is the failure that matters, because zero renders and absent does not (§8). Walk the sidebar (no item), the till (no badge, no receipt line, no line in the sync panel), the settings dialog (one paragraph, one `Conectar sistema`, no counts) and the digest inbox (nothing). Open `/fiscal-documents` directly: the never-populated empty state, neutral family, `Configurar`. The Panel walk belongs to S9's session and is re-run there; here the body shape stands in for it, which is exactly why that body carries no counts rather than zeros.
2. **Turning it on has a boundary (A3).** Connect the loopback target as the `owner`, through `Ajustes · Facturación electrónica` and not through a fixture or a shell — this is the named step that reaches the configured state, and walking it here is what also verifies the settings screen. `fiscal_documents` is still empty. Close two more sales: exactly two `pending` rows, both `created_at` after the group's `configured_at`. Then count the closed counter sales recorded before `configured_at` that have no document row — it must be the fifty, and none of them may appear on any of the work list's three lists.
3. **The payload reconciles (A4).** `GET /api/sales/{id}/canonical-document` for a seed sale carrying `excluded`, `rate_5` and `rate_19` lines. The sum of `lines[].line_total`, the sum of `lines[].tax_amount` and the `tax_by_class[]` bases must equal `totals`, and `totals` must equal `select subtotal, discount, tax, total from sales where id = …` exactly — integers in COP, no rounding tolerance. Then break it: move one line's `tax_amount` by one peso and close a sale on it. The row is `failed` with the arithmetic named in `error`, and the loopback's request log holds nothing at all for that `document_key`.
4. **The acquirer, in all three forms (A5, A6).** A sale with no customer renders `is_final_consumer: true` with `document_type`, `document` and `name` **absent from the object**, not present and null. A sale with a seed customer renders all three. A sale made with the machine's network interface down, to a customer registered at that same till while it was down, renders a complete block on reconnection with no field missing — the check is the payload after the push lands, not the till's local copy.
5. **A missing field never reaches the counter (A7).** Blank a seed customer's document number and close a sale for them. The row is `failed`, `Motivo` on `Fallidos` reads `El adquiriente no tiene número de documento` verbatim, and the till showed nothing at any point — walk it during the sale, not after. Restore the number and press `Reintentar`: the same row id moves to `acknowledged` with `attempts` incremented. One row throughout, no edit to it, no migration.
6. **Exactly once (A8) — the check this stage exists for.** Set the loopback to commit and then drop the connection. First attempt: the target holds the document, our row stays `pending`, `attempts` is 1. Let the ladder fire. For that one `document_key`, the target's request log must then hold exactly one `deliver` and one `query`, in that order, its store must hold exactly one document for that `sales.number`, and our row must be `acknowledged` carrying whatever the query returned. Two `deliver` entries — or two documents at the far end — is the failure the whole stage is shaped around, and it is visible in the log rather than inferred (§8, ledger).
7. **One row per sale, under concurrency (A10).** Call the handoff service twice concurrently for one closed sale, then group `fiscal_documents` by `document_key` and select the groups with a count above one: no rows. The queue holds one `deliver_fiscal_document` for that document, not two.
8. **A target that cannot dedupe is capped (A9).** Point the settings at a mapping declaring neither an idempotency field nor `query`, and force an ambiguous timeout. `attempts` stays 1, `next_attempt_at` is null, the row sits on the work list for a human, and the target's log holds exactly one request no matter how many sweeps run.
9. **A transport failure is never a refusal (A14).** Stop the loopback. `attempts` increments on 1 · 5 · 15 · 60 min, `status` stays `pending` at every step, and `Motivo` reads `No hay conexión con el sistema de facturación`. A row reading `Falló el envío` before the policy cap is the defect. Restart the target: the unsent count reaches zero with no manual step.
10. **The dwell re-queries, it does not re-send.** Set the loopback to *taken, confirmation later*: the row is `sent` with `sent_at` stamped. Past the dwell, the sweep's entry in the target's log must be a `query` and never a second `deliver`, and the row lands `acknowledged`.
11. **Held is not stuck (Offline · clock skew).** Push a sale whose `occurred_at` is outside the configured tolerance from `recorded_at`. The row exists with `next_attempt_at` null, `error` naming the skew, the row on `Pendientes`, and the target's log empty for it. Check the null and the empty log together — separately, held and stuck look identical.
12. **Returns and voids (A11, A12).** Return one line of an `acknowledged` sale: a second row whose `document_key` is `{location.code}-{sales.number}-NC1`, `references` naming the original's `sale_number` and `document_key`, `lines[]` holding only the returned line at the price on `sale_return_lines`, every amount positive. Then void a sale whose document has not left: the target's log shows the sale first and the credit note second, in that order, and it holds both. Two rows, neither deleted nor edited.
13. **Imported sales are refused (A13).** Set `source = imported` on one closed seed sale and call the handoff service directly in a pinned shell: it refuses, and `fiscal_documents` gains no row.
14. **Orphans are reported, never repaired (Jobs).** With a target configured, delete one document row directly and run the sweep. The sale appears on `Ventas sin enviar` with a reason in words, no row is recreated, and the count for that sale stays zero until a person acts.
15. **The file export (A16).** Run the export for a period twice: the two files are byte-identical, each `document_key` appears exactly once, every included document is `acknowledged`, and no key from an earlier period's file appears in a later one — compare the key sets, do not read them.
16. **The summary (A17).** `GET /api/fiscal-documents/summary` answers in under 50 ms p95 on the seed, and its unsent figure equals `select count(*) from fiscal_documents where status in ('pending','sent')`, per location and for the network.
17. **Audit, and the credential (A18).** A settings PATCH, a forced retry and a disconnect leave three `audit_log` rows with actor, entity and before/after. Then search those rows' `before` and `after` for the credential's value: zero hits. A credential in the audit log is a credential in a JSONB column by another route (§9).
18. **Roles and tenants (A19).** A `cashier` is offered no work-list entry and a direct link refuses inside the content region naming the role (§B.8.3). `GET /api/sales/{id}/fiscal-document` as that cashier, for a sale at their own sede, returns the identifiers and **no `status` key at all**; for another sede's sale it refuses. Seed the second tenant with **`seed_demo_tenant --profile minimal`** — the profile that exists so an isolation check has something to be isolated *from* (§1) — pin to it and select from `fiscal_documents`: zero rows. Then run the same select pinned back to `default`, where the session's own rows are all still there: without that second read the zero is indistinguishable from an empty database, which is how a seed that never ran reads as perfect isolation.
19. **The counter is untouched (A20, A21).** Delta pull with nothing pending under 20 ms p95, counter cold start under 2.5 s, and `Cobrar` to closed ticket under 200 ms p95 — each measured with the target unreachable and again with the network interface down, and each compared against the same number recorded before this stage's first commit. `blocked` is rendered nowhere: grep the bundle and walk the sync panel.

**The seed is a check, and "convincingly" means this.** **This stage registers one fixture with S0's command and ships no seeding path of its own** (§1, ledger cross-stage services): it declares `fiscal_documents` as the table it writes, answers all five profiles, and under every one of them writes no row and leaves the `invoicing` group empty. The registration is not a formality — it is what puts `fiscal_documents` under the guard, so a foreign row in it refuses the run, and it states the empty case explicitly rather than omitting it, which is what §1 asks of every fixture. **The unconfigured tenant is what the seed ships**, on `default` as on the other four, because that is what the product ships and the silence of §8 is the behaviour this stage most needs demonstrable from a bare seed. A seed that arrived configured would leave the default state rendered by nobody.

**The configured state is reached through the product's own settings screen, in a named step, and never by a second command or a sixth profile.** S0 owns the one seed command and §1 fixes its five profiles; a stage that shipped its own flip to a configured state would be the second seeding path the ledger forbids, and it would be the one path no reviewer reads because it is not the one an administrator walks. So: on `default`, as the `owner` — `target` is an `owner`-only key (§2) — open `Ajustes · Facturación electrónica` and connect the loopback target. **That step is check 2's opening move and the only configuration event in this document**, so nobody reads two of them into the session: it produces the state, verifies the settings surface in the same walk, and stamps `configured_at` exactly where check 2's boundary needs it.

Configured, and with the checks above run in order, `Envíos a facturación` must show rows across at least two sedes; at least three distinct `Motivo` strings on `Fallidos`, covering a target refusal — the loopback told to refuse — a validation failure (check 3) and a transport failure (check 9), each one produced by a check rather than typed into a row, and each a Spanish sentence a person could act on, never a bare HTTP status, never English, never a stack trace, never an empty cell; at least one row on `Ventas sin enviar` (check 14); all four `Estado` labels visible without filtering; and three record panels opened at random each showing a payload whose lines sum to its total. **`Creado` must not read one timestamp fifty times**: the rows the walk produces are spread across the session, so the oldest-unsent figure has something to say and the column is worth sorting. A wider spread comes from walking a `default` tenant seeded on an earlier day, never from a timestamp edited in place — that is the hand-editing §1 rules out, and it would make check 2's boundary untestable besides.

Unconfigured — the default the seed ships, and the state every demo opens in — the same route is one empty state and the settings section is one paragraph and one button, with no count, no badge and no warning family anywhere on any screen. **Check 1 is that walk and it runs before the named step**, because `configured_at` is stamped once and a disconnect does not un-stamp it: the shipped silence is walked first, or it is not walked at all in this session.

### What this stage would break

| Earlier behaviour | How S5 could break it | The one check |
|---|---|---|
| S4 · a sale closes | the handoff service runs inside the sale-close transaction, so anything it raises — unconfigured, invalid payload, an unresolvable credential — takes the sale down with it | close a sale at the till in three configurations: no target; a target whose credential does not resolve; a target pointed at an unroutable address. The sale commits and the receipt renders in all three |
| S4 · the `Cobrar` budget (§4) | delivery leaking onto the counter's critical path | `Cobrar` to closed ticket under 200 ms p95 with the target unreachable |
| S2 · a push applies a batch | the document write joins the push transaction and can fail the whole batch with it | push 50 offline sales in one batch: 50 sales land and 50 documents exist. No partial apply, no batch rejected for a document's sake |
| S2 · the delta pull and the registry (ledger rule 9) | a table of S5's reaching a device | the list of tables the till replicates is identical to before this stage, and delta pull with nothing pending stays under 20 ms p95 |
| S3 · the stock ledger (A3, ledger rule 7) | S5 writing a move — most plausibly on a credit note | the row count and latest `recorded_at` on `stock_moves` are identical before and after a full close → deliver → retry → return → export cycle |
| S1 · the catalog is read-only here | a builder that backfills a missing barcode, a blank description or a customer's name rather than failing validation | the latest `updated_at` on `items`, `item_barcodes` and `customers` is unchanged across that same cycle |
| S0 · `tenants.settings` (ledger rule 5) | writing the `invoicing` group by read-modify-write clobbers a neighbouring group | set a value in another settings group, save the invoicing settings, re-read the first: unchanged |
| S0 · tenant pinning (A1, ledger rule 6) | a job or an endpoint touching the database unpinned | run each of the four jobs and each new endpoint with no tenant pinned: each fails closed rather than reading across tenants |
| S0 · the role dependency (§2) | the settings PATCH treating `target` as an ordinary key | `admin` patching `target` is refused; `admin` patching `retry` succeeds; `owner` may do both |
