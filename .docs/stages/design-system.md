...
**Every other enum takes the same grammar.** `transfers` (`draft` neutral hollow · `dispatched` informative · `received` positive · `partial` warning), `sales` (`open` informative · `closed` positive · `voided` neutral solid), `price_proposals` (`proposed` neutral hollow · `approved` positive · `rejected` neutral solid · `above_cap` critical solid). A new enum picks a family per value against the five readings in §B.7.1 and adds nothing.
...
**1 · Precios (S7).** Filter bar + table + record panel, exactly Existencias' shape. Columns: `Producto 24 · Laboratorio 12 (left) · Costo 10 (right) · Precio 10 (right) · Margen 10 (right) · Propuesto 12 (right, editable) · Impacto mensual 12 (right) · Estado 10`. The `Propuesto` cell is §B.5.6's stepper. An item carrying `regulated_max_price` shows a hollow warning dot after its `Precio` value and states the cap in the panel; a proposal above the cap renders `Sobre el tope regulado` as a critical badge and its row cannot be included in an approval — the checkbox is absent, not disabled, and the reason is in the cell. Header: `Descartar` secondary, `Aplicar propuestas` primary. Bulk bar per §B.4.5. Four KPI tiles above the table: `Referencias con propuesta`, `Impacto mensual estimado`, `Margen proyectado`, `Propuestas sobre el tope`.
