...
| **The demo seed** — one command building a synthetic tenant that looks like the handoff, in the profiles §1 fixes | **S0** creates the command, the profile argument and the identity fixture (tenants, locations, users, invitations); **S1** registers the catalog | every other stage registers its own fixtures and is not finished until its screens render from the seed. The command is S0's because S0 creates the first seedable tables and its own verification needs a tenant to run against — a stage whose checks depend on a command a later stage builds cannot be verified in the session that builds it. Synthetic only, never loaded into a real tenant |
